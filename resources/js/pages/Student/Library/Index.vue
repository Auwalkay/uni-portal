...
const submitRequest = () => {
    requestForm.post('/student/library/request', {
        onSuccess: () => {
...
const triggerDownload = (bookId: number) => {
    window.open(`/student/library/books/${bookId}/download`, '_blank');
};
</script>

<template>
    <StudentLayout :breadcrumbs="[{ title: 'Library Catalog', href: '/student/library' }]">
        <Head title="Library Portal" />
