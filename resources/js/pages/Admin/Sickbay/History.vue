...
<template>
    <AdminLayout :breadcrumbs="[{ title: 'My Sickbay History', href: '/admin/sickbay/history' }]">
        <Head title="My Sickbay History" />
            ...
            <div class="relative overflow-hidden border-b bg-background px-6 py-12 md:px-12 lg:py-16">
                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between max-w-[1600px] mx-auto">
                    <div class="space-y-2">
                        ...
                        <h1 class="text-4xl font-extrabold tracking-tight lg:text-5xl">My Sickbay Hub</h1>
                        <p class="text-lg text-muted-foreground max-w-2xl">
                            Track your clinic visits, check-ins, medical treatment history, and emergency contact details.
                        </p>
                    </div>
                            ...
                            <div class="pl-2">
                                ...
                                <div v-if="visit.medical_log" class="mt-6 pt-4 border-t border-dashed grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                                    ...
                                    <div class="flex flex-col justify-center">
                                        ...
                                        <div class="text-muted-foreground p-2.5 bg-muted/40 rounded-lg border border-dashed flex items-center gap-1.5">
                ...
                <div v-if="currentTab === 'emergency'" class="max-w-2xl mx-auto space-y-6">
                    <Card class="border shadow-lg bg-card overflow-hidden">
                        ...
                        <CardContent class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm" v-if="staff">
                                <div class="space-y-1">
                                    <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Full Name</span>
                                    <span class="font-bold text-foreground block text-base">{{ staff.next_of_kin_name || 'Not Provided' }}</span>
                                </div>
                                <div class="space-y-1">
                                    <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Phone Number</span>
                                    <span class="font-bold text-foreground block text-base">{{ staff.next_of_kin_phone || 'Not Provided' }}</span>
                                </div>
                                <div class="space-y-1">
                                    <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Relationship</span>
                                    <Badge variant="outline" class="font-bold text-xs">{{ staff.next_of_kin_relationship || 'Not Provided' }}</Badge>
                                </div>
                                <div class="space-y-1">
                                    <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Staff ID Number</span>
                                    <span class="font-mono font-bold text-foreground block text-sm">{{ staff.staff_number }}</span>
                                </div>
                            </div>
                            <div v-else class="text-center p-6 text-sm text-muted-foreground">
                                No profile metadata found. (Admins/Superusers do not have a separate staff record table profile).
                            </div>

                            <div v-if="staff?.next_of_kin_address" class="space-y-1 border-t pt-4">
                                <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Home Address</span>
                                <span class="text-foreground text-sm font-semibold">{{ staff.next_of_kin_address }}</span>
                            </div>

                            <div v-if="staff?.department" class="space-y-1 border-t pt-4">
                                <span class="text-muted-foreground text-xs font-semibold uppercase tracking-wider block">Department / Unit</span>
                                <span class="text-foreground text-sm font-semibold">
                                    {{ staff.department }}
                                    <span v-if="staff.designation" class="text-muted-foreground">({{ staff.designation }})</span>
                                </span>
                            </div>
                            ...
                            <div class="p-4 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-900/30 rounded-xl flex gap-3 text-sm">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted-foreground text-xs mt-1 leading-relaxed">
                                        If these contact details are incorrect or have changed, please update them immediately in your profile settings.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
